...
class PostSpider(scrapy.Spider):
    ...
    request_comment_url_base = 'http://tieba.baidu.com/p/comment?tid=%s&pid=%s&pn=%s' # tid will be post_id,pid will be reply_id
    ...
    def parse(self, response):
        ...
        for sel in posts_sel:
            create_time = sel.css('.threadlist_author span.is_show_create_time::text').extract_first()
            last_reply_time = sel.css('.threadlist_author span.threadlist_reply_date::text').extract_first()
            create_time = str(create_time).strip()
            last_reply_time = str(last_reply_time).strip()

            print('-----------------------post-date&&last-reply-date-----------------------------')
            print(create_time)
            print(last_reply_time)

            if self.compare_post_date(create_time) or self.compare_post_date(last_reply_time): 
                post_id = sel.css('::attr(data-tid)').extract_first() # 直接取百度的id <a href="/p/6144256076?pid=125817093956&amp;see_lz=1#125817093956"></a>
                #posturl = "http://tieba.baidu.com/p/" + sel.css('.j_th_tit a::attr(href)').extract_first()
                #posturl = 'http://tieba.baidu.com/p/%s' % (post_id)
                author_home_url = self.request_baidu_domain + str(sel.css('.tb_icon_author a::attr(href)').extract_first())    
                item = PostItem()
                item['title'] = str(sel.css('.j_th_tit a::text').extract_first()).strip()
                item['post_id'] = post_id
                item['reply_num'] = str(sel.css('.threadlist_rep_num::text').extract_first()).strip()
                yield item
    ...
    def parse_reply(self, response):
        ...
        for sel in replies_sel:
            ...
            if len(sel.css('.tail-info::text').extract()) > 0:  # <span class="j_jb_ele"><a class="tail-info" data-checkun="un"><img class="icon-jubao" src=></a></span><span class="tail-info">2楼</span><span class="tail-info">2019-05-18 18:14</span>
                post_date = str(sel.css('.tail-info::text').extract()[-1]).strip()
            ...
            if self.compare_post_date(post_date[:7]):
                ...
                item['content'] = str(sel.css('.j_d_post_content::text').extract_first()).strip() #if is '',then leave it as ''
                if ''.join(sel.css('.post_bubble_middle_inner::text').extract()) != '': #有样式框的文本
                    item['content'] = ''.join(sel.css('.post_bubble_middle_inner::text').extract())

                author_home_url = self.request_baidu_domain + str(sel.css('.p_author_name::attr(href)').extract_first())
                yield item
    ...
    def parse_comment(self, response):
        ...
        for sel in comments_sel:            
            item = CommentItem()
            meta = response.meta
            author_home_url = self.request_baidu_domain + str(sel.css('.lzl_p_p::attr(href)').extract_first()).strip()
            
            item['post_id'] = response.meta['post_id']
            item['reply_id'] = response.meta['reply_id']
            item['content'] = ''.join(sel.css('.lzl_content_main::text').extract()).strip()
            yield item
            yield Request(author_home_url, callback=self.parse_user_related_tieba)
        ...
        if int(next_page) > int(response.meta['cur_page']): #meta.reply_id meta.post_id
    ...
    def parse_user_related_tieba(self, response):
        following_tieba_sel = Selector(response).css('.u-f-item::text').extract()
